...
public class RayCastFire : MonoBehaviour
{
    ...
    public Collider2D fireObjectCollider;
    ...
    void Update()
    {
        ...
        else if (time < 0 && GetComponentInParent<RandomSpawn>().FireActive == true)
        {
            allowSpread = true;
        }

        else if (GetComponentInParent<RandomSpawn>().FireActive == false && RandomSpawn.numberOfSpawners == 5)
        {
            fireObjectCollider.enabled = false; 
        }
    }
